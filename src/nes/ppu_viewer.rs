...
pub(crate) struct PpuViewer {
    ...
    mirroring: Mirroring,
    scanline: u32,
    nametables: [[u8; Self::NAMETABLE_SIZE]; 4],
    nametable_ids: [u8; Self::NAMETABLE_IDS_SIZE],
    pattern_tables: [[u8; Self::PATTERN_SIZE]; 2],
...
impl PpuViewer {
    ...
    fn new(window_id: WindowId, texture_id: TextureId) -> Self {
        Self {
            ...
            scanline: 0,
            nametables: [[0x00; Self::NAMETABLE_SIZE]; 4],
            nametable_ids: [0; Self::NAMETABLE_IDS_SIZE],
            pattern_tables: [[0x00; Self::PATTERN_SIZE]; 2],
...
impl Nes {
    ...
    pub(crate) fn render_ppu_viewer(&mut self, s: &mut PixState) -> PixResult<()> {
        if let Some(ref viewer) = self.ppu_viewer {
            s.with_window(viewer.window_id(), |s: &mut PixState| {
                s.clear()?;
                s.fill(Color::WHITE);
                s.stroke(None);
