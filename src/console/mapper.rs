use super::memory::{Addr, Byte, Memory};
...
pub fn new_mapper(cartridge: Cartridge) -> Result<Box<Memory>, Box<Error>> {
    match cartridge.mapper_num() {
        0 | 2 => Ok(Box::new(Nrom::new(cartridge))),
        // 1 => Ok(Box::new(SxRom::new(rom))),
        // 3 => Ok(Box::new(Mapper3::new(rom))),
        // 4 => Ok(Box::new(Mapper4::new(rom))),
        // 7 => Ok(Box::new(Mapper7::new())),
        _ => Err(format!("unsupported mapper : {}", cartridge.mapper_num()).into()),
    }
}
...
impl Nrom {
    pub fn new(cartridge: Cartridge) -> Self {
        let prg_banks = (cartridge.prg_size() / 0x4000) as u8; // 16K banks
...
impl Memory for Nrom {
    ...
    fn writeb(&mut self, addr: u16, val: u8) {
        match addr {
            0x0000...0x1FFF => self.cartridge.chr_rom[addr as usize] = val,
...
impl fmt::Debug for Nrom {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(
            f,
            "Nrom {{ PRG-RAM: {}KB, PRG Banks: {} }}",
            self.prg_ram.len() / 0x400,
            self.prg_banks
        )
    }
}
