...
#[derive(Clone)]
pub struct StaticMemory<T, const N: usize> {
    ram_state: RamState,
    data: [T; N],
}

impl<T, const N: usize> Default for StaticMemory<T, N>
where
    T: Default + Copy,
{
    fn default() -> Self {
...
impl<T, const N: usize> StaticMemory<T, N> {
...
impl<const N: usize> StaticMemory<u8, N> {
...
impl<const N: usize> Reset for StaticMemory<u8, N> {
...
impl<T, const N: usize> fmt::Debug for StaticMemory<T, N> {
...
impl<T, const N: usize> Deref for StaticMemory<T, N> {
    type Target = [T; N];
...
impl<T, const N: usize> DerefMut for StaticMemory<T, N> {
    fn deref_mut(&mut self) -> &mut Self::Target {
        &mut self.data
    }
}

impl<T: Serialize, const N: usize> Serialize for StaticMemory<T, N> {
...
impl<'de, T, const N: usize> Deserialize<'de> for StaticMemory<T, N>
where
