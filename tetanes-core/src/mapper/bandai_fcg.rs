...
impl BandaiFCG {
    ...
    pub fn load(cart: &mut Cart) -> Mapper {
        ...
        if bandai_fcg.mapper_num == 157 {
        ...
        } else if bandai_fcg.mapper_num == 16 {
            ...
            if bandai_fcg.submapper_num == 0
                || bandai_fcg.submapper_num == 5 && cart.prg_ram().len() == 256
            {
        ...
        } else if bandai_fcg.mapper_num == 16 && bandai_fcg.submapper_num == 4 {
            bandai_fcg.reg_access = MemoryOp::Read;
        } else if bandai_fcg.mapper_num == 16 && bandai_fcg.submapper_num == 5 {
            bandai_fcg.sram_access = MemoryOp::Read;
        }

        bandai_fcg.prg_rom_banks.set(1, 0x0F);
    ...
    fn write_irq_ctrl(&mut self, val: u8) {
        ...
        if self.mapper_num != 16 || self.submapper_num != 4 {
...
impl MemMap for BandaiFCG {
    ...
    fn map_write(&mut self, addr: u16, val: u8) -> MappedWrite {
        if matches!(addr, 0x6000..=0x7FFF)
            && !matches!(self.sram_access, MemoryOp::Write | MemoryOp::ReadWrite)
        {
