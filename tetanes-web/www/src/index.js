...
class State {
  ...
  setScale(scale) {
    ...
    this.imageData = this.p5.drawingContext.createImageData(this.nes.width(), this.nes.height());
  ...
  render() {
    ...
    this.imageData.data.set(new Uint8ClampedArray(memory.buffer, framePtr, frameLen));
    this.image.drawingContext.putImageData(this.imageData, 0, 0);
    this.p5.image(this.image, 0, 0, this.p5.pixelDensity() * this.p5.width, this.p5.pixelDensity() * this.p5.height);
  }
  ...
  playAudio() {
    ...
    if (samplesLen > 0) {
      ...
      const audioBuffer = this.audioCtx.createBuffer(1, 2048, this.sampleRate);
    ...
    document.getElementById('load-rom').addEventListener('change', function(e) {
      ...
      if (reader && files.length) {
        ...
        reader.onload = () => {
          ...
          state.setupAudio();
          p5.loop();
          document.getElementById('load-rom-label').textContent = 'Change ROM';
        };
      }
    }, false);
